...
export const Analytics: React.FC = () => {
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-3.5 rounded-xl shadow-md border border-slate-200 text-sm font-semibold text-slate-800">
          <p className="text-xs font-medium text-slate-400 mb-1">{payload[0].name}</p>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 bg-white rounded-2xl border border-slate-200">
        <div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight">Analityka finansowa</h2>
          <p className="text-xs text-slate-400 mt-0.5">Automatyczne zestawienia i proporcje wydatków</p>
        </div>
        <div className="flex items-center gap-2 bg-white p-1 rounded-xl border border-slate-200 self-start sm:self-auto">
          <button
            onClick={prevMonth}
            className="p-2 rounded-lg hover:bg-slate-50 text-slate-600 hover:text-slate-800 transition-all cursor-pointer"
            title="Poprzedni miesiąc"
          >
            &larr;
          </button>
          <span className="px-3 text-xs font-bold text-slate-700 min-w-32 text-center select-none flex items-center justify-center gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 flex flex-col justify-between">
          ...
          <p className="text-2xl font-bold text-slate-900 tracking-tight mt-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 flex flex-col justify-between">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h3 className="font-bold text-slate-800 tracking-tight text-sm uppercase">Struktura {chartView === 'expense' ? 'wydatków' : 'przychodów'}</h3>
              <p className="text-xs text-slate-400 mt-0.5">Podział procentowy według kategorii</p>
            </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 flex flex-col justify-between">
          <div className="mb-4">
            <h3 className="font-bold text-slate-800 tracking-tight text-sm uppercase">Porównanie budżetu</h3>
            <p className="text-xs text-slate-400 mt-0.5">Zestawienie przychodów i wydatków</p>
          </div>

          {totalIncome === 0 && totalExpenses === 0 ? (
            <div className="h-64 flex flex-col items-center justify-center text-center p-6 bg-slate-50/50 rounded-xl border border-dashed border-slate-200">
              <TrendingUp className="w-8 h-8 text-slate-300 mb-2" />
              <p className="text-xs font-semibold text-slate-500">Brak danych do porównania</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <Tooltip 
                    ...
                    content={({ active, payload }) => {
                      if (active && payload && payload.length) {
                        return (
                          <div className="bg-white p-3 rounded-xl shadow-md border border-slate-200 space-y-1 text-xs font-semibold">
                            <p className="text-slate-800 font-bold border-b border-slate-100 pb-1">Miesięczne Zestawienie</p>
                            {payload.map((p, idx) => (
                              <div key={idx} className="flex items-center gap-4 justify-between">
                                ...
                                <span className="font-bold text-slate-800">{formatCurrency(p.value as number)}</span>
                              </div>
                            ))}
                          </div>
                        );
