    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs transition-opacity duration-300">
      ...
      >
        ...
        <div className="p-5 flex justify-between items-center border-b border-slate-50 bg-slate-50">
          ...
          <button 
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-slate-100 transition-colors text-slate-400 cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              autoFocus
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 placeholder-slate-400 focus:outline-hidden focus:border-indigo-500 focus:bg-white transition-all"
            />
          </div>

          {/* Color Palette Selector */}
          <div>
            <label className="block mb-3 text-xs font-semibold text-slate-500">
